                ...
                {dataMovies.map((movie, index) => {
                    return (
                        <Link to={routes.profileLink(movie.id)} key={index} >
                            <button onClick={() => routes.profileLink(movie.id)} className='relative bg-[#0d131f] font-medium shadow-lg flex-col lg:hover:opacity-80 overflow-hidden 
                          text-start rounded-lg' >
                                ...
                                <div className='p-3 '>
                                    ...
                                    <div className="text-[16px]">{movie.id}</div>
