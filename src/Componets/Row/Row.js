                        ...
                        <Link to={routes.detailLink(mediaType, movie?.id)} key={index} >
                           ...
                           rounded-lg overflow-hidden '>
                                ...
                                <div className='bg-secondary text-[14px] p-3  flex justify-between'>
                                    ...
                                    <div className='text-ellipsis  whitespace-nowrap overflow-hidden flex flex-col justify-between '>
                                        <div className="text-ellipsis overflow-hidden ">{movie?.title || movie?.name}</div>
                                        <div> <RemoveRedEyeOutlinedIcon color='warning' /> {Math.floor(movie?.popularity)}</div>
                                    </div>
                                    <div className='flex flex-col ml-2 justify-between'>
                                        <Button variant='outlined' size='small' className=" min-w-[20px] text-Red border-Red text-[12px]">{(movie?.release_date || movie?.first_air_date).split('-')[0]}</Button>
